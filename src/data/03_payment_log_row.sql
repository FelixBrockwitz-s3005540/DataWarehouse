-- Procedural accounts.payment_log_row population
...
--   - Every paid accounts.abo is billed MONTHLY at the plan price from Lizenzmodelle.txt
--     (stored on its accounts.payment_details row, which we reuse here).
--   - The log covers the billing HISTORY of each accounts.abo: one entry per month,
--     going backwards from now. Entries older than the current accounts.abo's
...
BEGIN
    ...
    LOOP
        ...
        FOR v_k IN 0..v_n - 1 LOOP
            ...
            INSERT INTO accounts.payment_log_row (timestamp, abo_id, payment_details_id,
    ...
    SELECT count(*) INTO v_count FROM accounts.payment_log_row;
